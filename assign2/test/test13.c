...
void print_key_set(struct key_set key_set){
	int i;
	printf("K: \n");
	for (i=0; i<8; i++) {
...
void generate_sub_keys(unsigned char* main_key, struct key_set* key_sets) {
	int i, j;
	int shift_size;
...
void process_message(unsigned char* message_piece, unsigned char* processed_piece, struct key_set* key_sets, int mode) {
	int i, k;
	int shift_size;
